...
export class LocationFieldAltitudeMaximumComponent {
        ...
        this._subscription = this._geolocationService
            ...
            .subscribe((position) => {
                if (
                    position &&
                    position.success &&
                    !isNaN(position.altitudeMaximum)
                ) {
                    this.value = this._distanceService.metersToString(
                        position.altitudeMaximum,
                        {
                            useSmallUnits: true,
                        }
                    );
...
component('location-field-altitude-maximum', {
    style: css``,
    template: html`
        ...
        >
    `,
}, LocationFieldAltitudeMaximumComponent);
